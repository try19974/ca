...
class WeaponDetectorNode(Node):
    def __init__(self):
        ...
        # JSON logging
        global _json_file
        _json_file = open("detections.json", "w", encoding="utf-8")
    ...
    def publish_detections(self, detections):
        ...
        for d in detections:
            ...
            point_msg = Point()
            point_msg.x = float(d["horiz_mm"]) if d["horiz_mm"] is not None else 0.0
            point_msg.y = float(d["vert_mm"]) if d["vert_mm"] is not None else 0.0
            point_msg.z = float(d["z_mm"]) if d["z_mm"] is not None else 0.0
            self.point_pub.publish(point_msg)
            
            # Log to file
            global _json_file
            if _json_file is not None:
                _json_file.write(json.dumps(record) + "\n")
                _json_file.flush()
